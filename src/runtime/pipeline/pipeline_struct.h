...
class BackendRuntime : public BasicRuntime {
 private:
  ...
  bool LoadBindingData(int input_index) {
	...
	//std::cout <<"stage"<<runtime_idx_<<" wait input :"<< std::chrono::duration_cast<std::chrono::nanoseconds>(tt2 - tt1).count() << " ns or" \
 ...
 public:
  ...
  bool RunPipeline() {
	...
	//std::cout <<"stage"<<runtime_idx_<<" forward time of"<<pipeline_execution_count_<<" :"<< std::chrono::duration_cast<std::chrono::nanoseconds>(ttt2 - ttt1).count() << " ns or" \
        <<std::chrono::duration_cast<std::chrono::nanoseconds>(ttt2 - ttt1).count()/1000000<< " ms" << std::endl;
