...
if env.TARGET not in ["sim", "tsim", "intelfocl"]:
    ...
    vta.program_fpga(remote, bitstream="/home/eddie/gemm_ultra96.bit")
...
def get_network():
    ...
    mvalue2 = np.full((1), 1).astype("float32")
    mvalue3 = np.full((1), 1).astype("float32")
    ...
    net1_output2 = net1_output1
    net1_output3 = net1_output2
    #(net1_output3,_) = relay.split(net1_output3, indices_or_sections=2, axis=0)
    #net1_output3 = relay.add(net1_output3, mv1)
    ...
    net2 = relay.add(net1_output3, mv1)
    net2 = net2
    net2_output = net2
    ...
    net3 = relay.add(net2_output, mv1)
...
def get_split_mod():
    ...
    print(mod)
    split_conf = [{"op_name": "add", "op_index": 0}, {"op_name": "add", "op_index": 1}]
...
if __name__ == "__main__":
    if pipeline_executor_build.pipeline_executor_build_enabled():
        ...
        for target in target_list:
            ...
            datas = []
            for i in range(3):
                datas.append(np.full(dshape, 0.1 + i).astype("float32"))
            ...
            test1 = vta.build_config(opt_level=0)
    ...
    #============================================================================================
            ...
            # Use the import function to create and initialize PipelineModule.
            pipeline_module_test = pipeline_module
            assert pipeline_module_test.num_outputs == 1
            ...
            for round in range(0, len(datas)):
                ...
                end1 =  time.time()
                print("Execution time of overhead：%f" % (end1 - start1))
                
                '''
                assembly = pipeline_module_test.module.get_source
                with open("/home/eddie/out1.txt", "w") as txtfile:
                    print(assembly, file=txtfile)
                '''
                #start1 = time.time()
                #print("start timer")
                pipeline_module_test.run()
            ...
            print("Execution time of all：%f" % (end - start))
