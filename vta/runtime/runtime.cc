...
struct DataBuffer {
   ...
   * \param size The size of the data.
   */
  void FlushCache1(size_t offset, size_t size) {
    //if (!kBufferCoherent && kAlwaysCache) {
      VTAFlushCache1(reinterpret_cast<char*>(data_) + offset, phy_addr_ + offset, size);
    //}
  }
...
void VTAtest(VTACommandHandle cmd){
	return;
}
...
unordered_map <std::thread::id,int> mp;

ofstream out("output.txt");

void VTAconv2d(VTACommandHandle cmd, int num, float* conv2d_input_1, float* conv2d_input_2, float* conv2d_output,
						int conv2d_input_1_shape0, int conv2d_input_1_shape1, int conv2d_input_1_shape2, int conv2d_input_1_shape3,
						int conv2d_input_2_shape0, int conv2d_input_2_shape1, int conv2d_input_2_shape2, int conv2d_input_2_shape3,
						int conv2d_output_shape0, int conv2d_output_shape1, int conv2d_output_shape2, int conv2d_output_shape3, int turn){
	cout<<"!!!!!!!!!!!!!!!!!!!!!!!!!!!conv!!!!!!!!!!!!"<<endl;
	
	int i,j,k,l,m,n;
	
	
	int stride=conv2d_input_1_shape3/conv2d_output_shape3;
	int padding=(conv2d_output_shape3*stride+conv2d_input_2_shape3-1)-conv2d_input_1_shape3;
	padding=padding/2;
	
	int H=conv2d_input_1_shape2;
	int W=conv2d_input_1_shape2;
	int ich=conv2d_input_2_shape1;
	int och=conv2d_input_2_shape0;
	int ksize=conv2d_input_2_shape2;
	
	cout<<conv2d_input_1_shape0<<" "<<conv2d_input_1_shape1<<" "<<conv2d_input_1_shape2<<" "<<conv2d_input_1_shape3<<endl;
	cout<<conv2d_input_2_shape0<<" "<<conv2d_input_2_shape1<<" "<<conv2d_input_2_shape2<<" "<<conv2d_input_2_shape3<<endl;
	cout<<conv2d_output_shape0<<" "<<conv2d_output_shape1<<" "<<conv2d_output_shape2<<" "<<conv2d_output_shape3<<endl;
	...
	if(mp.find(std::this_thread::get_id())==mp.end()){
		stage_count++;
		mp[std::this_thread::get_id()]=stage_count;
	}
	
	turn=mp[std::this_thread::get_id()];
	//cout<<turn<<endl;
	
	//vta::DataBuffer* src1 = vta::DataBuffer::FromHandle(conv2d_input_1);
	//vta::DataBuffer* src2 = vta::DataBuffer::FromHandle(conv2d_input_2);
	//vta::DataBuffer* dst1 = vta::DataBuffer::FromHandle(conv2d_output);
	
	
	//float* input1=(float*)VTAMemAlloc1(sizeof(float)*(conv2d_input_1_shape0*conv2d_input_1_shape1*conv2d_input_1_shape2*conv2d_input_1_shape3+2*base),0);
	//float* input2=(float*)VTAMemAlloc1(sizeof(float)*(conv2d_input_2_shape0*conv2d_input_2_shape1*conv2d_input_2_shape2*conv2d_input_2_shape3),0);
	//float* output=(float*)VTAMemAlloc1(sizeof(float)*(conv2d_output_shape0*conv2d_output_shape1*conv2d_output_shape2*conv2d_output_shape3),0);
	
	
	//memcpy(input1+base,conv2d_input_1,sizeof(float)*(conv2d_input_1_shape0*conv2d_input_1_shape1*conv2d_input_1_shape2*conv2d_input_1_shape3));
	//memcpy(input2,conv2d_input_2,sizeof(float)*(conv2d_input_2_shape0*conv2d_input_2_shape1*conv2d_input_2_shape2*conv2d_input_2_shape3));
	...
	//for(int i=0;i<10;i++)cout<<*(float*)(conv2d_output+i*sizeof(float))<<endl;
	
	return;
}

void VTAstart_timer(VTACommandHandle cmd, int num, int is_conv2d){
...
void VTAstop_timer(VTACommandHandle cmd, int num, int is_conv2d){
	
	tt2 = Clock::now();
	
	std::cout << "OP " <<num<<" execution time:"<< std::chrono::duration_cast<std::chrono::nanoseconds>(tt2 - tt1).count() << " ns or" \
        <<std::chrono::duration_cast<std::chrono::nanoseconds>(tt2 - tt1).count()/1000000<< " ms" << std::endl;
	
	if(num==1)sum=0;
	sum+=std::chrono::duration_cast<std::chrono::nanoseconds>(tt2 - tt1).count();
	cout<<sum<<" "<<sum/1000000<<endl;
	
	out<<is_conv2d<<" "<<std::chrono::duration_cast<std::chrono::nanoseconds>(tt2 - tt1).count()/1000000<<" "<<sum/1000000<<endl;
...
void VTA_print_conv2d(VTACommandHandle cmd,
						int conv2d_input_1_shape0, int conv2d_input_1_shape1, int conv2d_input_1_shape2, int conv2d_input_1_shape3,
						int conv2d_input_2_shape0, int conv2d_input_2_shape1, int conv2d_input_2_shape2, int conv2d_input_2_shape3,
						int conv2d_output_shape0, int conv2d_output_shape1, int conv2d_output_shape2, int conv2d_output_shape3){
	//usleep(50*1000);
	...
	cout<<conv2d_input_1_shape1<<endl;
	cout<<conv2d_input_1_shape2<<endl;
	cout<<conv2d_input_1_shape3<<endl;
	cout<<conv2d_output_shape1<<endl;
	cout<<conv2d_output_shape2<<endl;
	cout<<conv2d_output_shape3<<endl;
	cout<<conv2d_input_2_shape2<<endl;
