...
def InjectALUIntrin():
    ...
    def _ftransform(func, mod, ctx):
        ...
        if(loop.attrs.global_symbol.find("conv2d")!=-1):
            is_conv2d=True
        ...
        if(False):
            ...
            print(loop)
            '''
            print(loop.buffer_map[func.params[0]].data)
            print(loop.buffer_map[func.params[0]].shape)
            print(loop.buffer_map[func.params[1]].data)
            print(loop.buffer_map[func.params[1]].shape)
            print(loop.buffer_map[func.params[2]].data)
            print(loop.buffer_map[func.params[2]].shape)
            '''
            '''
            print(dir(loop))
            for bff in loop.buffer_map:
                print(bff)
                print(bff.legacy_repr)
                print(bff.dtype)
                print(dir(bff))
            print(loop.attrs)
            print("---------------------------------")
            print(loop.body)
            print(dir(loop.body))
            print(loop.body.buffer_var)
            print(dir(loop.body.buffer_var))
            print("---------------------------------")
            print(loop.body.body)
            print(dir(loop.body.body))
            print(loop.body.body.buffer_var)
            print(dir(loop.body.body.buffer_var))
            print("---------------------------------")
            print(loop.body.body.body)
            print(dir(loop.body.body.body))
            '''
            
        #print(loop.checked_type)
        
        
        #pre 24 37
        
        #if(num==9 or num==23 or num==24 or num==26 or num==36 or num==37 or num==39 or num==49 or num==50 or num==52 or num==62 or num==63):
        #if(num==23 or num==36 or num==49 or num==62):
        #if(num==9):
        turn=0
        ...
        if(is_conv2d):
            ...
            irb.emit(
                tvm.tir.call_extern(
                    ...
                    env.dev.command_handle,
                    num,
                    func.buffer_map[func.params[0]].data,
                    func.buffer_map[func.params[1]].data,
                    func.buffer_map[func.params[2]].data,
                    func.buffer_map[func.params[0]].shape[0],
                    func.buffer_map[func.params[0]].shape[1],
                    func.buffer_map[func.params[0]].shape[2],
                    func.buffer_map[func.params[0]].shape[3],
                    func.buffer_map[func.params[1]].shape[0],
                    func.buffer_map[func.params[1]].shape[1],
                    func.buffer_map[func.params[1]].shape[2],
                    func.buffer_map[func.params[1]].shape[3],
                    func.buffer_map[func.params[2]].shape[0],
                    func.buffer_map[func.params[2]].shape[1],
                    func.buffer_map[func.params[2]].shape[2],
                    func.buffer_map[func.params[2]].shape[3],
                    turn,
                )
            )
            func.body=irb.get()
        ...
        '''
        if(is_conv2d):
        #if(is_conv2d and num!=23 and num!=36 and num!=49 and num!=62):
        #if(is_conv2d and num!=9):
            irb = tvm.tir.ir_builder.create()
            irb.emit(
                tvm.tir.call_extern(
                    "float32",
